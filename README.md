...
**Metodología de Arquitectura**: **Why-Driven Design (WDD)** | **Modelo C4** | **Clean Architecture (.NET 9)** | **CQRS & Patrón Mediator (MediatR)** | **Pipeline Behaviors** | **Performance Engineering (500 TPS)** | **ISO/IEC 25010**  
...
Todos los 7 ejercicios pueden ejecutarse de manera independiente y en simultáneo sin conflictos de puertos:
...
### Para probar el Ejercicio 6 (Performance Engineering 100 a 500 TPS):
```bash
cd pregunta_06
cat JUSTIFICACION_TECNICA.md
```

### Para probar el Ejercicio 7 (CQRS & Patrón Mediator .NET 9 - Puerto 8007):
```bash
cd pregunta_07
podman compose up -d --build
./load_test/run_visual_demo.sh
...
2. Selecciona **"Abrir diagrama existente"** y selecciona cualquiera de los archivos del repositorio:
   ...
   - Diagrama C4 Model Ejercicio 7: `pregunta_07/diagrams/cqrs_mediator_architecture.drawio`
...
- **Consistencia Eventual vs. Consistencia Fuerte Inmediata**: Aceptamos consistencia eventual de segundos en canales de notificación (Email/SMS) y proyecciones de lectura a cambio de garantizar respuestas en **0.23 ms** y un SLA de disponibilidad del 99.99%.
...
- **Caching Distribuido vs Escalado Vertical Ciego de BD**: Incurrimos en administrar Redis Caching con TTL a cambio de reducir el 80% de lecturas a la BD relacional y lograr pasar de 100 TPS a 500 TPS con latencias P95 < 50ms.
- **Desacoplamiento MediatR CQRS vs CRUD Monolítico**: Incurrimos en separar Commands y Queries en clases específicas a cambio de permitir lecturas ultrarrápidas (< 2ms) y escalado independiente de la base de datos de lectura.
